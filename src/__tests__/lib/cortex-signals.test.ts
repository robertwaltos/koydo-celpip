...
import { emitPrismSignal, trackQuestionAttempt } from "@/lib/celpip/cortex-signals";

describe("Cortex Signals", () => {
  it("dispatches a custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ event: "test", examId: "EXAM017" });
  ...
  it("tracks question attempts", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    trackQuestionAttempt("EXAM017", "q1", true, 5000);
    expect(spy).toHaveBeenCalledTimes(1);
